# YellowOS for ESP32-2432S028

YellowOS is a lightweight Windows-inspired application environment for the
...
The current firmware is **Roadmap Stage 0: hardware diagnostics**. It does not
yet contain the desktop or application runtime. Its job is to establish a
reliable hardware baseline for this exact board before the OS is built on top.
...
5. Open `OperationSystem.ino` from this folder.
...
```text
OperationSystem.ino       Arduino entry point only
partitions.csv            4 MiB flash layout
src/board/                board pin map and hardware drivers
src/diagnostics/          Stage 0 test application
docs/HARDWARE.md          known and unverified hardware facts
docs/STAGE_0.md           exact test procedure and acceptance criteria
docs/ARCHITECTURE.md      long-term YellowOS boundaries
docs/ROADMAP.md           development stages
```

## Safety

Stage 0 creates `/yellowos_stage0.tmp` and `/yellowos_stress.tmp` on the SD
card and removes them at the end. Back up an important card before testing and
do not remove power or eject the card during an SD write.
